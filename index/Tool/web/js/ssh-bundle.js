var MiniServerSSH = (() => {
  ...
  function encodePacket(payload, blockSize) {
    ...
    // 随机填充，符合 RFC 4253 §6（H10 修复）
    var padding = new Uint8Array(paddingLength);
    globalThis.crypto.getRandomValues(padding);
    return concat(writeUint32(packetLength), writeUint8(paddingLength), payload, padding);
  }
  ...
  // DER INTEGER 编码（BigInt → DER）
  function derInt(bigint) {
    var hex = bigint.toString(16);
    if (hex.length % 2) hex = "0" + hex;
    var bytes = new Uint8Array(hex.length / 2);
    for (var i = 0; i < bytes.length; i++) bytes[i] = parseInt(hex.substr(i * 2, 2), 16);
    if (bytes[0] & 0x80) {
      var p = new Uint8Array(bytes.length + 1);
      p.set(bytes, 1);
      bytes = p;
    }
    return concat2(new Uint8Array([0x02]), derLen(bytes.length), bytes);
  }
  // SSH 公钥 blob → SPKI DER（支持 ssh-rsa / rsa-sha2-256 / rsa-sha2-512）
  function rsaSshKeyToSpki(publicKeyBlob, hashAlg) {
    var off = 0;
    var algR = readSshString(publicKeyBlob, off);
    off = algR.offset;
    var eR = readMpint(publicKeyBlob, off);
    off = eR.offset;
    var nR = readMpint(publicKeyBlob, off);
    var rsaPub = concat2(derInt(nR.value), derInt(eR.value));
    rsaPub = concat2(new Uint8Array([0x30]), derLen(rsaPub.length), rsaPub);
    var algOid;
    if (hashAlg === "SHA-1") algOid = new Uint8Array([0x30,0x0d,0x06,0x09,0x2a,0x86,0x48,0x86,0xf7,0x0d,0x01,0x01,0x05,0x05,0x00]);
    else if (hashAlg === "SHA-256") algOid = new Uint8Array([0x30,0x0d,0x06,0x09,0x2a,0x86,0x48,0x86,0xf7,0x0d,0x01,0x01,0x0b,0x05,0x00]);
    else algOid = new Uint8Array([0x30,0x0d,0x06,0x09,0x2a,0x86,0x48,0x86,0xf7,0x0d,0x01,0x01,0x0d,0x05,0x00]);
    var bitStr = concat2(new Uint8Array([0x03]), derLen(rsaPub.length + 1), new Uint8Array([0x00]), rsaPub);
    return concat2(new Uint8Array([0x30]), derLen(algOid.length + bitStr.length), algOid, bitStr);
  }
  // RSA 主机密钥验证（解析 SSH 格式 blob）
  async function verifyRsaSha256(publicKeyBlob, signatureBlob, data) {
    var algR = readSshString(publicKeyBlob, 0);
    var algName = new TextDecoder().decode(algR.value);
    var hashAlg;
    if (algName === "ssh-rsa") hashAlg = "SHA-1";
    else if (algName === "rsa-sha2-256") hashAlg = "SHA-256";
    else if (algName === "rsa-sha2-512") hashAlg = "SHA-512";
    else throw new Error("Unsupported host key alg: " + algName);
    var spki = rsaSshKeyToSpki(publicKeyBlob, hashAlg);
    // 签名 blob: string alg, string sig
    var sAlgR = readSshString(signatureBlob, 0);
    var sSigR = readSshString(signatureBlob, sAlgR.offset);
    var publicKey = await subtle.importKey("spki", spki, { name: "RSASSA-PKCS1-v1_5", hash: hashAlg }, false, ["verify"]);
    return subtle.verify("RSASSA-PKCS1-v1_5", publicKey, sSigR.value, data);
  }
  ...
  async function deriveKey(K, H, sessionId, letter, length) {
    ...
    // K 必须按 mpint 编码（含 4 字节长度前缀），RFC 4253 §7.2
    var kBytes = writeMpint(K);
  ...
  // 常量时间字节比较，防止 MAC 校验时的时序攻击（H8 修复）
  function byteEq(a, b) {
  ...
  var SSHTransport = class {
    ...
    route(payload) {
      ...
      if (type === SSH_MSG_CHANNEL_DATA || type === SSH_MSG_CHANNEL_EXTENDED_DATA) {
        ...
        if (ch) {
          ...
          // 通道窗口调整：本地窗口不足时通知对端，避免 >2MB 传输卡死（H9 修复）
          ch.localWindow = (ch.localWindow || WIN_SIZE) - d.data.length;
          if (ch.localWindow < WIN_SIZE / 2) {
            var delta = WIN_SIZE - ch.localWindow;
            ch.localWindow = WIN_SIZE;
            this.sendPayload(concat4(
              writeUint8(SSH_MSG_CHANNEL_WINDOW_ADJUST),
              writeUint32(ch.remoteId),
              writeUint32(delta)
            ));
          }
        }
      ...
      if (type === SSH_MSG_CHANNEL_WINDOW_ADJUST) {
        // 远端通知窗口增加，更新对端窗口以允许继续发送（H9 修复）
        var adjCh = this.channels.get(u32(payload, 1));
        if (adjCh) adjCh.remoteWindow = (adjCh.remoteWindow || 0) + u32(payload, 5);
        return;
      }
    ...
    async openSessionChannel() {
      ...
      var ch = {
        localId: localId,
        remoteId: 0,
        localWindow: WIN_SIZE,
        remoteWindow: 0,
        remoteMaxPkt: MAX_PKT,
        dataCbs: [],
        extCbs: [],
        resolveEof: rEof,
        resolveClose: rClose,
      ...
      // 解析 CHANNEL_OPEN_CONFIRMATION: sender_channel, initial_window_size, max_packet_size
      ch.remoteId = u32(reply, 1);
      ch.remoteWindow = u32(reply, 5);
      ch.remoteMaxPkt = u32(reply, 9);
      return ch;
    }
    ...
    async execCommand(command) {
      ...
      await ch.eofPromise;
      // 退出状态超时从 500ms 提升到 5000ms，避免慢命令误判
      await Promise.race([ch.exitPromise, new Promise(function(r) { setTimeout(r, 5000); })]);
