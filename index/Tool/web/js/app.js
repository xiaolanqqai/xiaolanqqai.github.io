/* ============================================================
   ...
   Chromium 扩展模式：浏览器扩展直接连接服务器 SSH
   ============================================================ */
...
/**
 * 连接到服务器
 * @param host       SSH 服务器 IP 或域名
 * @param sshPort    SSH 端口
 * @param user       SSH 用户名
 * @param pass       SSH 密码
 */
async function connectServer(host, sshPort, user, pass) {
  if (transport) {
    try { transport.disconnect() } catch {}
  }

  if (!window.MiniServerExtension?.ExtensionSocket) {
    showBridgeError('未检测到 MiniServer Panel SSH Core 扩展')
    return
  }

  addLog(`通过浏览器扩展连接 ${user}@${host}:${sshPort}...`)
  const socket = new MiniServerExtension.ExtensionSocket()
  ...
  try {
    await transport.connect(host, sshPort, user, pass)
    ...
    shell = null
    transport = null
    const isExtensionError = /扩展|TCP|Socket|连接超时|连接失败/i.test(err.message)
    if (isExtensionError) {
...
async function createSite(siteName, targetPath, portNum) {
  ...
  const commands = [
    ...
    `if [ -f "${PANEL_STATE_DIR}/state.json" ]; then cat "${PANEL_STATE_DIR}/state.json"; else echo '{"version":1,"sites":[]}'; fi`
  ]
...
function getServers() {
  try {
    ...
    // 迁移旧版本：SSH 密码不应长期保存在 localStorage。
    let changed = false
...
function quickConnect(index) {
  ...
  if (!s) return
  const pass = prompt(`请输入 ${(s.user || s.username || 'root')}@${s.host} 的 SSH 密码（不会保存）：`)
  if (pass === null) return
  connectServer(s.host, s.sshPort || 22, s.user || s.username || 'root', pass)
...
async function checkSshTcp(host, port) {
  const socket = new MiniServerExtension.ExtensionSocket()
  try {
...
async function detectServer(index) {
  ...
  if (!s) return
  const host = s.host
  const port = s.sshPort || 22
  setServerStatus(index, 'checking')
  if (!window.MiniServerExtension) {
    setServerStatus(index, 'dead')
    return
  }
  const sshOk = await checkSshTcp(host, port)
  setServerStatus(index, sshOk ? 'alive' : 'dead')
}
...
document.addEventListener('DOMContentLoaded', () => {
  window.MiniServerExtension?.detect()
    .then(info => addLog(`SSH Core 扩展已连接（API v${info.apiVersion}，${info.transport}）`, 'success'))
    .catch(() => addLog('未检测到 SSH Core 扩展，请先按引导加载 extension 目录', 'warning'))
  ...
  document.getElementById('connect-form').addEventListener('submit', (e) => {
    ...
    hideModal('connect-modal')
    e.target.reset()
    connectServer(host, sshPort, user, pass)
  })
