...
echo ""
echo -e "${GREEN}Token 固定为 ssh，无需手动填写。${NC}"
echo ""
...
if command -v systemctl >/dev/null 2>&1 && [ -d /etc/systemd/system ]; then
    ok "systemd 可用"
    REPLY=""
    read -p "是否配置 systemd 服务实现开机自启？[Y/n] " -n 1 -r || true
    echo ""
    if [[ ! $REPLY =~ ^[Nn]$ ]]; then
...
EOF
        ...
        REPLY=""
        read -p "是否立即启动服务？[Y/n] " -n 1 -r || true
        echo ""
        if [[ ! $REPLY =~ ^[Nn]$ ]]; then
            ...
            if systemctl is-active --quiet mini-bridge.service; then
                ok "服务已启动"
                echo ""
                echo -e "${GREEN}访问 Token: $BRIDGE_TOKEN${NC}"
                echo ""
                echo -e "${BLUE}管理命令：${NC}"
...
# ─── 7. 询问是否立即运行（前台） ──────────────────
REPLY=""
read -p "是否现在前台启动 bridge？[Y/n] " -n 1 -r || true
echo ""
if [[ ! $REPLY =~ ^[Nn]$ ]]; then
    info "启动 bridge ..."
    exec ./bridge
