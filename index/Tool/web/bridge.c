...
static int local_origin(const char *origin, size_t len, const char *base) {
    size_t base_len = strlen(base);
    return len >= base_len && memcmp(origin, base, base_len) == 0 &&
           (len == base_len || origin[base_len] == ':');
}

static int origin_allowed(const char *buf) {
    const char *origin = strstr(buf, "\r\nOrigin: ");
    if (!origin) origin = strstr(buf, "\r\norigin: ");
    if (!origin) return 0;
    origin += 10;
...
static int parse_ws_frame(const char *buf, int len, int *payload_start, int *opcode, int *masked) {
    ...
    if (len < 2) return -1;
    if ((u[0] & 0x70) != 0) return -2; /* 未协商任何 WebSocket 扩展 */
    *opcode = u[0] & 0x0f;
    *masked = (u[1] & 0x80) ? 1 : 0;
    if (!*masked) return -2; /* RFC 6455: 客户端帧必须掩码 */
    int plen = u[1] & 0x7f;
    int hdr = 2;
    if (plen == 126) {
        if (len < 4) return -1;
        ...
        if (len < 10) return -1;
        if (u[2] || u[3] || u[4] || u[5] || (u[6] & 0x80)) return -2;
        unsigned long long long_len =
            ((unsigned long long)u[2] << 56) | ((unsigned long long)u[3] << 48) |
            ((unsigned long long)u[4] << 40) | ((unsigned long long)u[5] << 32) |
            ((unsigned long long)u[6] << 24) | ((unsigned long long)u[7] << 16) |
            ((unsigned long long)u[8] << 8) | u[9];
        if (long_len > 1024 * 1024) return -2;
        plen = (int)long_len;
        hdr = 10;
    }
    if (*masked) {
...
static int handle_ws_handshake(int fd, char *buf, int len, const char *token) {
    ...
    if (path_len != (int)strlen(token) || memcmp(path, token, path_len) != 0)
        return -1;
    if (!origin_allowed(buf)) return -1;
...
static void handle_ws_message(client_t *c, int epfd, const char *payload, int len) {
    ...
    if (strcmp(type, "connect") == 0) {
        ...
        if (c->tcp_fd >= 0) {
            send_json(c->fd, "error", NULL, "TCP connection already exists");
            return;
        }
...
int main(int argc, char *argv[]) {
    ...
    if (argc > 2) {
        if (strlen(argv[2]) < 3 || strlen(argv[2]) >= sizeof(g_token)) {
            fprintf(stderr, "token must be 3-32 characters\n");
            return 1;
        }
    ...
    printf("╔══════════════════════════════════════════════╗\n");
    printf("║    MiniServer Panel Bridge v3.5              ║\n");
    printf("╠══════════════════════════════════════════════╣\n");
    printf("║  监听端口: %-34d  ║\n", port);
    printf("║  Token: %-37s  ║\n", g_token);
    ...
    printf("╠══════════════════════════════════════════════╣\n");
    printf("║  安全提示:                                  ║\n");
    printf("║  · 监听 0.0.0.0:8022，请限制可访问来源       ║\n");
    printf("║  · 默认 Token 为 ssh，前端会自动使用。       ║\n");
    printf("║    也可通过第二参数指定 16-32 位 Token：      ║\n");
    ...
    while (1) {
        ...
        for (i = 0; i < num_clients; i++) {
            if (clients[i].fd >= 0 && now - clients[i].last_activity > IDLE_TIMEOUT)
                close_client(epfd, &clients[i]);
        }
        ...
        for (i = 0; i < n; i++) {
            if (events[i].data.fd == listen_fd) {
                ...
                if (events[i].events & EPOLLOUT) {
                    ...
                    if (getsockopt(tcp_fd, SOL_SOCKET, SO_ERROR, &socket_error, &error_len) < 0 ||
                        socket_error != 0) {
                ...
                if (c->state == ST_WS_HANDSHAKE) {
                    /* HTTP 握手可能被拆成多个 TCP 包，先累积再解析。 */
                    c->last_activity = time(NULL);
                    if (c->ws_buf_len + len > MAX_HTTP_HEADER) {
                        const char *err = "HTTP/1.1 431 Request Header Fields Too Large\r\n"
                                          "Content-Length: 0\r\n\r\n";
                        send_all(ws_fd, err, strlen(err));
                        close_client(epfd, c);
                        continue;
                    }
                    if (c->ws_buf_len + len + 1 > c->ws_buf_cap) {
                        int new_cap = c->ws_buf_cap ? c->ws_buf_cap * 2 : 2048;
                        while (new_cap < c->ws_buf_len + len + 1) new_cap *= 2;
                        char *nb = realloc(c->ws_buf, new_cap);
                        if (!nb) {
                            close_client(epfd, c);
                            continue;
                    ...
                    memcpy(c->ws_buf + c->ws_buf_len, buf, len);
                    c->ws_buf_len += len;
                    c->ws_buf[c->ws_buf_len] = '\0';
                    int ret = handle_ws_handshake(ws_fd, c->ws_buf, c->ws_buf_len, g_token);
                    if (ret < 0) {
                        /* Token 错误或握手失败 */
