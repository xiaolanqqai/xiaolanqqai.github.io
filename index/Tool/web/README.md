...
一个面向低配 VPS（≥100MB RAM）的极简远程服务器管理面板。前端部署在
GitHub Pages，Chromium 扩展提供直接 TCP 连接，SSH 协议栈运行在浏览器中。
服务器只需系统原有的 `sshd`，没有面板常驻进程。

> 当前为扩展直连 SSH 的技术原型。正式公网使用前，需要补齐 SSH Key、
> known_hosts 主机密钥确认和 SFTP。
...
> GitHub Pages 强制 HTTPS，因此浏览器只能连接 `wss://`。C bridge 本身提供
> `ws://`，生产环境必须使用带有效域名证书的 Caddy/Nginx 反向代理到
> `127.0.0.1:8022`。前端连接时填写该域名和 TLS 端口（通常为 443）。
