...
BETA 4.4.1-2026-03-17
1：版本管理系统修复：修复了 vol-manager.html 中模态框结构错位导致的 addEventListener 报错及添加版本功能失效问题
2：GitHub 同步增强：优化了 manager-common.js 中的 getFromGithub 函数，采用 TextDecoder 处理 Base64 解码，彻底解决中文字符乱码导致的解析失败
3：稳定性优化：改进了 GitHub 文件的 404 错误处理逻辑，将文件缺失由报错降级为日志警告，提升探索系统的初始化稳定性

BETA 4.4-2026-03-17
1：导航管理系统修复：修复添加新选项卡时因 navData 结构访问错误导致的 "Cannot read properties of undefined (reading 'some')" 报错
2：逻辑完善：修正 addNewTabWithName 函数中的数据路径，支持多页面（index, more, dd等）的选项卡独立添加
3：体验优化：补全缺失的 selectTab 功能，实现新选项卡添加后自动切换并聚焦，同时修正新选项卡初始数据结构（sites -> items）以匹配全局 JSON 规范
