...
- **局域网工具页**：新增 `index/Tool.html`，支持按网段与常见 Web 端口搜索局域网内可访问网页
...
- **工具模块化**：`Tool.html` 采用“页面骨架 + UI 组件 + 扫描逻辑”拆分，分别由页面文件、`js/components/tool-scanner-component.js` 与 `js/tool-scanner.js` 组成
...
- **推荐环境**：若需使用 `Tool.html` 搜索局域网 `http://192.168.x.x:端口` 页面，建议通过局域网内本地 HTTP 服务访问本项目，而不是直接从公网 HTTPS 页面访问

### Tool 页面限制说明
- **浏览器安全策略**：若页面从 `https://` 打开，浏览器通常会限制或拦截对局域网 `http://` 地址的主动探测
- **结果性质**：Tool 页面采用浏览器无授权探测的尽力模式，发现到的地址可直接访问，但未发现不代表目标服务一定不存在
